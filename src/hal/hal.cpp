...
//    Serial.print("dio[2]: "); Serial.println(plmic_pins->dio[2]);
    ...
    if (lmic_pins.rxtx != LMIC_UNUSED_PIN) {
        // initialize to RX
//        digitalWrite(lmic_pins.rxtx, LOW != lmic_pins.rxtx_rx_active);
...
// val == 1  => tx
 void hal_pin_rxtx (u1_t val) {
     //if (lmic_pins.rxtx != LMIC_UNUSED_PIN)
//         digitalWrite(lmic_pins.rxtx, val != lmic_pins.rxtx_rx_active);
}
...
// Interrupt handling
//--------------------
static const unsigned NUM_DIO_INTERRUPT = 3;
...
static void hal_spi_init () {
 ...
 static void hal_spi_trx(u1_t cmd, u1_t* buf, size_t len, bit_t is_read) {
     uint32_t spi_freq;
     u1_t nss = lmic_pins.nss;

 //     if ((spi_freq = lmic_pins.spi_freq) == 0)
